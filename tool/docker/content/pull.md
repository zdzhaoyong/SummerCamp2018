...
上面的命令中没有给出 Docker Registry 地址，因此将会从 Docker Hub 获取镜像。而镜像名称是 `ubuntu:16.04`，因此将会获取官方镜像 `library/ubuntu` 仓库中标签为 `16.04` 的镜像。
...
*如果从 Docker Hub 下载镜像非常缓慢，可以参照 [镜像加速器](prepare.md) 一节配置加速器。*
...
root@062c39a594ad:/# cat /etc/os-release
NAME="Ubuntu"
...
进入容器后，我们可以在 Shell 下操作，执行任何所需的命令。这里，我们执行了 `cat /etc/os-release`，这是 Linux 常用的查看当前系统版本的命令，从返回的结果可以看到容器内是 `Ubuntu 16.04.3 LTS` 系统。

最后我们通过 `exit` 退出了这个容器。


### 下一节: [操作容器](container.md)
